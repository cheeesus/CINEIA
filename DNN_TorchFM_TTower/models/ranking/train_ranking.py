"""
Train custom DeepFM ranker using 4 dense features:
...
def main(epochs=3, batch_size=2048, neg_ratio=1):
    ...
    # During training, recall_score is not available -> fill with 0.0
    df["recall_score"] = 0.0
    ...
    for ep in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for Xs, Xd, y in val_loader:
                Xs, Xd, y = Xs.to(device), Xd.to(device), y.to(device)
                val_loss = loss_fn(model(Xs, Xd), y).item()
                total_val += val_loss * y.size(0)
                count_val += y.size(0)
